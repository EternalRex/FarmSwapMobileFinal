...
class SaveConsumerCartOrder {
  Future<void> saveCartOrder(
/*Consumer Details */
    String consName,
    String consLName,
    String consUname,
    ...
    var sellListingRef =
        FirebaseFirestore.instance.collection("sample_SellListings").doc(docId);
    var buyCollectionRef =
        sellListingRef.collection('sell').doc(listingId).collection('sellbuy');
    await buyCollectionRef.add(saveOrder.toMap());
    // Remove from 'sellcart'
    var sellCartRef =
        sellListingRef.collection('sell').doc(listingId).collection('sellcart');

    // Get the documents in the sell cart collection
    var sellCartSnapshot = await sellCartRef.get();

    // Add a print statement
    print('Attempting to remove from sellcart');

    // Delete each document in the sell cart collection
    for (var doc in sellCartSnapshot.docs) {
      await doc.reference.delete();
      print('Document deleted from sellcart');
    }
  }
}

//removing product to cart
class RemoveConsumerCartOrder {
  Future<void> removeCartOrder(
/*Consumer Details */
    String consName,
    String consLName,
    String consUname,
